...
async function main()
{
  ...
  const sourceCode = await fs.readFile('burningToken.sol', 'utf8');
  // Compile the source code and retrieve the ABI and bytecode
  const { abi, bytecode } = compile(sourceCode, 'burningToken') //you may alter to point to a contract that you have
  // Store the ABI and bytecode into a JSON file
  const artifact = JSON.stringify({ abi, bytecode }, null, 2);
  await fs.writeFile('burningToken.json', artifact);
}
