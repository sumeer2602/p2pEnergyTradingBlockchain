...
const web3 = new Web3(
  new Web3.providers.IpcProvider(
    "/home/ubuntu/ForIBFT/Node0/data/geth.ipc",
    net
  )
); //get the web3 injection from where your ipc file is located
...
async function main() {
  ...
  await createContract(contractAbi, contractByteCode, 47, myAccount) //call the createContract function using the contractAbiy, bytecode, constructor and deployer address
    ...
    .catch(console.error);
  console.log(new Date().getTime() - timeBefore);
  //onsole.log('contract address', tAddress) //this is address of the contract

  var contAddress =
    //a simple struct of the address, we will save this as a JSON file called contAddress.json
    {
      address: tAddress,
    };
